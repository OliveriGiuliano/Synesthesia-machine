...
It is deliberately implementation-oriented. It does not merely describe desired features: it fixes the major technical decisions, defines runtime and data contracts, specifies the behaviour of the initial node catalogue, establishes repository boundaries, and divides the project into independently implementable phases.

The phase documents accompanying this master document are intended to be handed to a coding agent one at a time. Each phase packet repeats the contracts required for that phase, states which files may be changed, and includes acceptance tests. The coding agent should not need the entire repository in context.
...
Early phases may execute the engine in-process behind the same interface, but no UI code may depend on that implementation detail.
...
The implementation sequence is deliberately vertical. It establishes contracts before breadth and proves the complete video-to-note path before implementing every node.
...
Every phase packet supplied to a coding agent shall contain:

1. phase objective and explicitly excluded work;
...
- avoid broad refactors outside the phase scope;
...
- phase packets reference files, not entire directories, where possible;
...
- agents should be asked to implement one coherent sub-phase at a time, such as three related image nodes, rather than the entire catalogue in one run.
...
Mitigation: dependency lock, early Phase 0 packaging spike, standalone build before installer work, bundled notices, and a formal licensing review before external distribution. FFmpeg codec configuration and Qt LGPL obligations must be reviewed for the chosen distribution model; this document is not legal advice.
...
6. **Self-Modulated Rotation**  
   ...
   Note: the original vision uses average saturation to modulate rotation, so the catalogue should add general Channel Statistics nodes—Mean, Minimum, Maximum, Standard Deviation, Percentile—early in Phase 5 even though they were not explicitly listed. These output FLOAT and are broadly necessary for parameter modulation.
...
Following the phased plan will produce useful, testable software by Phase 3 and reduce the risk of implementing dozens of nodes on top of an unproven runtime.

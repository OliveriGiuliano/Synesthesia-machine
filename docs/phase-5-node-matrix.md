# Phase 5 node implementation matrix

This matrix fixes the implementation identities and shared policies for Phase 5 before coding. It
is derived from the Phase 5 packet and sections 7, 8, 9, 10, 11.3–11.4, 16, 18.7, 19, 21, and 24
of the master architecture. Display labels may evolve; the type, port, parameter, and enum IDs in
this document are persistent identities.
...
- `nodes/image/dimensions.py`, `adjustments.py`, `filters.py`, `channels.py`, `utilities.py`, and
  ...
  stable display order. The Phase 3 `core.py` facade remains import-compatible while its contents
  are split.
...
- `tests/phase5/conftest.py` and `tests/phase5/conformance.py` provide synthetic frames, immutable
...
## Scope clarification recorded by the matrix

Section 19 requires every Image Adjustment node in section 16. The packet's abbreviated Batch 2
list omits the three explicit scalar arithmetic definitions described in section 16.4; they are
therefore included at the end of Batch 2 rather than silently dropped. Threshold remains in Batch 4
as ordered by the Phase 5 packet.
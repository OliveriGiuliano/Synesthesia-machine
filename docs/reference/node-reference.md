# Synesthesia node reference

The production registry includes four visual-to-musical algorithms and three MIDI-state utilities.
They operate on immutable runtime values, return desired `MidiStateFrame` state, and never
...
source-drop cases are covered in `tests/nodes/synesthesia` and `tests/integration`. Repeatable
500×500 microbenchmark evidence is generated at `docs/evidence/synesthesia-benchmarks.json`.

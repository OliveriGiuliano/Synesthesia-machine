...
**Acceptance:**
- [ ] No constructor parameter anywhere in the pipeline is stored but never used
- [ ] Exactly one module routes a pumped batch; the routing inputs are assembled by that module
- [ ] The window applies sink decisions (pill/dock/panel) but performs no routing
- [ ] Preview tests pass; `uv run check` green; no unrelated diff

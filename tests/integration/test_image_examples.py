"""Persisted image-pipeline example and compatibility-catalogue tests."""
...
REFERENCE_PATH = Path("examples/library/reference-image-pipeline.synmachine.json")
REFERENCE_MEDIA_PATH = Path("examples/library/media/reference.mp4")
...
def test_reference_graph_loads_and_compiles_with_exact_image_branches() -> None:
    registry = create_application_registry()
...
def test_image_artifacts_have_canonical_schema_round_trips() -> None:
    registry = create_application_registry()

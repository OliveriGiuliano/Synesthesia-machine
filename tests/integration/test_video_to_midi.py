"""Saved graph and generated video-to-MIDI integration tests."""
...
EXAMPLE_PATH = Path("examples/library/hue-chord.synmachine.json")
...
def _materialize_example(root: Path) -> Path:
    ...
    generate_hue_test_video(
        graph_path.parent / "media" / "hue-chord.mp4",
        width=64,
        height=64,
...
def test_saved_example_is_portable_complete_and_audio_opt_in(tmp_path: Path) -> None:
    assert (EXAMPLE_PATH.parent / "media" / "hue-chord.mp4").is_file()
    ...
    assert (
        ...
        == (graph_path.parent / "media" / "hue-chord.mp4").resolve()
    )
    ...
    assert Path(persisted_source["parameters"]["file_path"]) == Path("media/hue-chord.mp4")

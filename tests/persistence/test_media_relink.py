"""Portable media identity and undoable relinking tests."""
...
def _saved_video_graph(root: Path) -> tuple[Path, Path]:
    ...
    media_path.write_bytes(b"portable-video-identity" * 4096)
    graph_path = root / "workspace.synmachine.json"
    document = GraphDocument()
    document.add_node(
        "synmachine.input.load_video",
        parameters={"file_path": str(media_path), "playback_speed": 0.5},
    )
...
def test_saved_media_is_relative_and_survives_project_tree_relocation(tmp_path: Path) -> None:
    ...
    assert source["parameters"]["playback_speed"] == 0.5
    ...
    assert snapshot.nodes[0].parameters["playback_speed"] == 0.5

...
These exercise ``compute_demand_roots`` directly against published graph
view models — no main window, no event loop, no engine — so the ADR-0008/0011
demand policy is covered without the offscreen-window and timing machinery the
transport tests use.
"""
...
def _view_model(document: GraphDocument, registry: NodeRegistry) -> GraphViewModel:
    """Project the document into the view model the demand policy consumes."""
...
def test_visualizer_docks_gate_demand_roots() -> None:
    ...
    view = _view_model(document, registry)

    assert set(compute_demand_roots(view, image_dock_visible=True, note_dock_visible=True)) == {
        image_id,
        note_id,
    }
    assert set(compute_demand_roots(view, image_dock_visible=True, note_dock_visible=False)) == {
        image_id
    }
    assert set(compute_demand_roots(view, image_dock_visible=False, note_dock_visible=True)) == {
        note_id
    }
    assert compute_demand_roots(view, image_dock_visible=False, note_dock_visible=False) == ()
...
def test_image_link_pill_keeps_producer_when_dock_hidden() -> None:
    ...
    assert display_id in compute_demand_roots(
        view, image_dock_visible=True, note_dock_visible=False
    )
    ...
    roots = compute_demand_roots(view, image_dock_visible=False, note_dock_visible=False)

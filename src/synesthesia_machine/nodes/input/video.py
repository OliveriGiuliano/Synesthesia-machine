...
def _load_video_parameter_editor(
    ...
    if not isinstance(file_path, str) or not file_path or facts is None or facts.file_path is None:
        return spec
    if str(normalize_media_path(facts.file_path)) != str(normalize_media_path(file_path)):
        return spec
            ...
            execution=NodeExecutionContract(
                ...
                (
                    ...
                    ParameterSpec(
                        ...
                        PortType.INT,
                        0,
                        help_text="Index of the video stream inside the file; 0 is the first "
                        "stream.",
                        minimum=0,
                        update_mode=ParameterUpdateMode.RESTART_SOURCE,
                    ),

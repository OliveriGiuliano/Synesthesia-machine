...
class DebugSynth:
    ...
    def __init__(
        ...
        *,
        ...
        # One shared state-ordering policy (ADR-0022): state frames whose tick
        # carried a generation at or below the panic watermark were generated
        # before the panic and must not re-arm the voices the panic silenced.
    ...
    def update(self, state: MidiStateFrame) -> None:
        ...
        with self._publisher_lock:
            if self._closed:
                return
            if self._ordering.is_stale(state.context.publish_generation):
                return
            notes = state.notes
            if not notes:
...
class NullDebugSynth:
    ...
    Records panics through the shared state-ordering policy (ADR-0022), so its
    watermark stays comparable to the real synth's.
    """

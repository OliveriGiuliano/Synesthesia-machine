"""Placement-independent engine body: the facade/worker/source orchestration
that both engine placements run.

The in-process placement wraps it in the in-process protocol client; the
spawned child runs it directly behind the wire, where the event publisher
drives its note/value preview publication seam (ADR-0026). It owns the
preview cursors, so a new activation generation renumbers preview sequences
from 1 in both placements without any wrapper having to remember to reset
them.
"""
...
class EngineBody:
    """The placement-independent engine body both engine placements run (ADR-0026).

    The in-process placement wraps it in the in-process protocol client;
    the spawned child runs it directly behind the wire, where the event
    publisher drives its note/value preview publication seam.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # "Already delivered up to this sequence" cursors: the body owns
    ...
    def activate(
        ...
        *,
        ...
        with self._lock:
            ...
            # generation's counters. Both placements drive these cursors
            # (the in-process session pump through the client, the child's
            # event publisher straight through this body), so the reset
            # lives on the body itself: there is no wrapper to keep in
            # sync.
    ...
    @property
    def graph_revision(self) -> int | None:
        with self._lock:
            return self._graph_revision

    def last_activation(self) -> tuple[GraphSnapshot, tuple[UUID, ...] | None] | None:
        """The last successfully activated plan, as a restart source.

        The snapshot and its demand roots are read together under the body
        lock so a concurrent activation cannot split the pair.
        """
    ...
    def _ensure_open(self) -> None:
        if self._state is EngineState.CLOSED:
            raise RuntimeError("Engine body is closed")

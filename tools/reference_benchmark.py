"""Benchmark the 500x500/60 FPS reference graph through the real UI and child engine."""
...
DEFAULT_OUTPUT = Path("docs/evidence/reference-benchmark.json")
DEFAULT_REFERENCE_GRAPH = Path("benchmarks/fixtures/reference-500x500.synmachine.json")
...
def create_reference_document(media_path: str) -> GraphDocument:
    """Create the deterministic two-branch reference graph."""
...
def write_reference_graph(path: Path = DEFAULT_REFERENCE_GRAPH) -> Path:
    ...
    save_graph(path, create_reference_document("synthetic-500x500-60fps.mp4").snapshot())
    return path

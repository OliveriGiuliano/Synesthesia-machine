"""Run the accelerated Hold Image bounded-memory soak and record evidence."""
...
def _compiled_scheduler(*, delay_frames: int, memory_limit_mb: int) -> Scheduler:
    ...
    if compilation.plan is None:
        ...
        raise RuntimeError(f"Hold Image soak graph did not compile: {messages}")
...
def _frame(context: FrameContext, *, width: int, height: int) -> ImageFrame:
    ...
    return ImageFrame(
        ...
        AlphaMode.NONE,
        context,
        FrameProvenance(SOURCE_ID, "hold-image-accelerated-soak"),
    )
...
def main() -> int:
    ...
    print(
        f"Hold Image accelerated soak: ticks={report.methodology.total_ticks}, "

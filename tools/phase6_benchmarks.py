"""Benchmark the four Phase 6 synthesis algorithms on deterministic 500x500 fixtures."""
...
DEFAULT_OUTPUT_PATH = Path("docs/phase-6-benchmarks.json")
...
def run_benchmarks(
    *,
    ...
    report = Phase6BenchmarkReport(
        ...
        methodology=BenchmarkMethodology(
            scope=(
                "Direct Qt-free Phase 6 algorithm calls over pre-built immutable synthetic "
...
def _image(values: NDArray[np.float32], context: FrameContext) -> ImageFrame:
    return ImageFrame(
        ...
        AlphaMode.NONE,
        context,
        FrameProvenance(SOURCE_ID, "phase6-benchmark"),
    )
...
def main() -> int:
    ...
    print(f"Phase 6 500x500 microbenchmarks: {summary}; passed={report.passed}")

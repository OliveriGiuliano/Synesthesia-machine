"""Run the canonical Phase 3 graph in the real Qt UI and record diagnostic evidence."""
...
DEFAULT_OUTPUT_PATH = Path("docs/phase-3-performance.json")
DEFAULT_SCREENSHOT_PATH = Path("docs/phase-3-ui.png")
...
class _PerformanceRun(QObject):
    def __init__(
        ...
        *,
        ...
        self.report: Phase3PerformanceReport | None = None
    ...
    def _create_report(
        ...
        return Phase3PerformanceReport(
...
def _prepare_runtime_graph(
    ...
    *,
    ...
    source_path = runtime_root / "phase3-performance-500x500.mp4"
    ...
    runtime_graph_path = runtime_root / "phase3-performance.synmachine.json"
...
def run_diagnostic(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="synmachine-phase3-performance-") as temporary:
        ...
        application = create_application(["synmachine-phase3-performance"])
        registry = create_application_registry()
        ...
        if not window.open_path(runtime_graph_path):
            client.close()
            raise RuntimeError("Could not open the temporary Phase 3 diagnostic graph")
...
def main() -> int:
    ...
    print(
        f"Phase 3 diagnostic: {report.engine.processed_fps:.2f} processed FPS, "

"""Run the accelerated 60-minute-equivalent Phase 8 memory/profiler/MIDI safety soak."""
...
from tools.phase5_soak import run_soak

DEFAULT_OUTPUT = Path("docs/phase-8-soak.json")
...
MIDI_PORT_NAME = "Phase 8 Mock Loopback"
    ...
    def send(self, message: MidiMessage) -> None:
        if message.message_type is MidiMessageType.NOTE_ON and not self._blocked:
            ...
            if not self.release.wait(5.0):
                raise TimeoutError("Phase 8 MIDI overload did not release its sender")
        super().send(message)
...
def run_phase8_soak(*, output_path: Path, total_ticks: int = DEFAULT_TICKS) -> Phase8SoakReport:
    if total_ticks < 120:
        raise ValueError("Phase 8 soak requires at least 120 ticks")
    warmup_ticks = min(6_000, total_ticks // 3)
    sample_interval = max(1, total_ticks // 12)
    with tempfile.TemporaryDirectory(prefix="synmachine-phase8-soak-") as temporary:
...
def main() -> int:
    ...
    report = run_phase8_soak(output_path=args.output, total_ticks=args.ticks)

"""Measure the canonical graph in the real Qt UI with a spawned engine process."""
...
DEFAULT_OUTPUT_PATH = Path("docs/evidence/process-ui-diagnostic.json")
DEFAULT_SCREENSHOT_PATH = Path("docs/evidence/process-ui-diagnostic.png")
...
@dataclass(frozen=True, slots=True)
class ProcessUiDiagnosticReport:
...
class _PerformanceRun(QObject):
    def __init__(
        ...
        *,
        ...
        self.report: ProcessUiDiagnosticReport | None = None
    ...
    def _create_report(
        ...
        return ProcessUiDiagnosticReport(
            ...
            methodology=BenchmarkMethodology(
                scope=(
                    ...
                    "reference image graph because Gaussian Blur and Canny are not implemented"
                ),
...
def _prepare_runtime_graph(
    ...
    *,
    ...
    source_path = runtime_root / "process-ui-diagnostic-500x500.mp4"
    ...
    runtime_graph_path = runtime_root / "process-ui-diagnostic.synmachine.json"
...
def run_diagnostic(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="synmachine-process-ui-diagnostic-") as temporary:
        ...
        application = create_application(["synmachine-process-ui-diagnostic"])
        registry = create_application_registry()
        client = ProcessEngineClient(
        ...
        if not window.open_path(runtime_graph_path):
            client.close()
            raise RuntimeError("Could not open the temporary process UI diagnostic graph")
...
def main() -> int:
    ...
    print(
        f"Process UI diagnostic: {report.engine.processed_fps:.2f} processed FPS, "
